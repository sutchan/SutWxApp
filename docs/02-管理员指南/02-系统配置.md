...
本文档适用于SutWxApp微信小程序的开发环境、测试环境和生产环境的系统配置管理。
...
`app.json`是小程序的全局配置文件，包含了小程序的页面路径、全局样式、导航栏样式、底部标签栏样式等配置。
...
`sitemap.json`用于配置小程序及页面是否允许被微信索引。
...
项目使用`.env`系列文件管理不同环境的配置变量，详见《环境搭建》中的环境变量配置部分。
...
1. **集中管理**：将所有配置集中到统一的config目录
2. **环境分离**：为不同环境创建独立的配置文件
3. **安全存储**：敏感配置使用环境变量或加密配置文件
4. **版本控制**：配置文件纳入版本管理，但排除包含敏感信息的文件
...
| project.config.json | 项目配置配置 | 项目根目录 |
