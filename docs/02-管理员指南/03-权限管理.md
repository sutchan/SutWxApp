...
本文档详细说明SutWxApp微信小程序项目的权限管理体系，包括权限类型设计、角色定义、权限分配、权限验证机制以及安全最佳实践，为系统管理员提供完整的权限管理参考。
...
控制用户对系统功能模块的访问权限，如用户管理、产品管理、订单管理等。

#### 2.2.2 数据权限

控制用户对特定数据资源的访问范围，如只能查看自己创建的订单，或只能管理特定类别的产品等。
...
- `product:category:create` - 创建产品分类
...
在后台管理系统中配置不同用户组及对应的权限和权益。

#### 4.3.2 权限验证机制

小程序端实现权限验证逻辑，控制用户对特定功能和内容的访问：
...
function logPermissionOperation(userId, userName, operation, resource, result) {
  const logEntry = {
    ...
    userId,
    userName,
    operation,         // 操作类型，如create_role, assign_permission等
    resource,          // 操作资源，如role_id, permission_id等
    result,            // 操作结果，如success, failed
...
3. **会话管理**：合理设置会话超时，避免会话保持
4. **安全日志**：记录所有关键操作，定期审计
...
| product | list | view | product:list:view | 查看产品列表 |
| product | detail | view | product:detail:view | 查看产品详情 |
| product | basic | create | product:basic:create | 创建产品基本信息 |
| product | basic | edit | product:basic:edit | 编辑产品基本信息 |
