...
async function getPointsBalance(): Promise<PointsBalance> {
  try {
    const result = await request.get<PointsBalance>('/points/balance') as PointsBalance;
    return result;
  } catch (error) {
...
async function getPointsHistory(params: PointsHistoryParams = {}): Promise<PointsHistoryResult> {
  try {
...
async function getPointsTasks(params: PointsTasksParams = {}): Promise<PointsTasksResult> {
  try {
...
async function claimTaskReward(taskId: string): Promise<ClaimResult> {
  try {
    return await request.post<ClaimResult>(`/points/tasks/${taskId}/claim`) as ClaimResult;
  } catch (error) {
...
async function completeTask(taskId: string, data: Record<string, unknown> = {}): Promise<CompleteResult> {
  try {
    return await request.post<CompleteResult>(`/points/tasks/${taskId}/complete`, data) as CompleteResult;
  } catch (error) {
...
async function usePoints(amount: number, reason: string, data: UsePointsParams = {}): Promise<UsePointsResult> {
  try {
...
async function exchangeProduct(productId: string, quantity = 1): Promise<ExchangeProductResult> {
  try {
    return await request.post<ExchangeProductResult>('/points/exchange/product', {
      productId,
      quantity
...
async function getExchangeProducts(params: ExchangeProductsParams = {}): Promise<ExchangeProductsResult> {
  try {
    return await request.get<ExchangeProductsResult>('/points/exchange/products', {
      page: 1,
      pageSize: 20,
      ...params
...
async function getPointsStats(params: PointsStatsParams = {}): Promise<PointsStatsResult> {
  try {
