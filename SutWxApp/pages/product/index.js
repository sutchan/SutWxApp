...
Page({
    ...
    wx.request({
      ...
      success: function(res) {
        if (res.data && res.data.success) {
          ...
          const defaultSpecIndex = productInfo.specs && productInfo.specs.length > 0 ? 0 : 0;
    ...
    wx.request({
      ...
      success: function(res) {
        if (res.data && res.data.success) {
          ...
          const newReviews = that.data.reviewsPageNum === 1 ? list : [...that.data.reviews, ...list];
          that.setData({
            reviews: newReviews,
  ...
  handleQuantityChange: function(e) {
    ...
    if (type === 'minus') {
      ...
      selectedQuantity = Math.min(this.data.productInfo.stock || 99, selectedQuantity + 1);
    }
  ...
  handleQuantityInput: function(e) {
    ...
    quantity = Math.max(1, Math.min(this.data.productInfo.stock || 99, quantity));
    ...
    wx.request({
      ...
      success: function(res) {
        if (res.data && res.data.success) {
          ...
          wx.showToast({ title: res.data.message || '操作失败', icon: 'error' });
        }
      }
    });
  },
