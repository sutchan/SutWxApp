...
> 本项目为纯前端微信小程序，不含本地数据库。内容源为 **WordPress 网站（headless CMS）**，经 REST API 提供；前端通过 `services/*` 读取与提交，本地仅持久化 Token、用户信息、购物车等轻量数据。WordPress 侧的数据模型（文章、页面、分类、媒体、自定义文章类型等）是其自身 Schema，小程序只消费并按需映射。因此本规范聚焦「前端数据实体建模」「本地存储」「接口数据契约」与「数据安全」，不涉及数据库建表、事务与索引。
...
#### WordPress 内容实体映射
除电商实体外，内容展示的核心来自 WordPress。前端应将其内容类型映射到小程序数据实体：

| WordPress 内容类型 | REST 资源 | 小程序对应 | 说明 |
|-------------------|-----------|------------|------|
| 文章（Post） | `posts` | 文章/资讯详情 | 正文为 HTML，需渲染层处理 |
| 页面（Page） | `pages` | 单页内容 | 关于、帮助等静态页 |
| 分类（Category） | `categories` | 分类导航 | 对应 `pages/category` |
| 标签（Tag） | `tags` | 标签筛选 | 可选 |
| 媒体（Media） | `media` | 图片/视频 | WP 上传的资源 |
| 自定义文章类型 | 自定义 | 业务内容 | 如 WooCommerce 的 `products`、`orders` |
