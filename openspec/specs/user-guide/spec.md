...
项目为纯前端微信小程序，代码集中在 `SutWxApp/` 目录下，遵循微信小程序标准目录结构，按 `pages/`、`components/`、`services/`、`utils/`、`locales/` 组织。
...
商品搜索支持关键词搜索、分类筛选、价格区间筛选和排序功能。搜索结果按照综合排序、价格排序和销量排序展示，用户可以灵活调整排序方式。商品详情页展示商品图片、价格、规格、库存、评价和销量等信息，支持加入购物车、立即购买和收藏操作。商品数据可来自 WordPress 的 WooCommerce 插件；当 `app.js` 的 `globalData.productSource` 设为 `woocommerce` 时，商品列表与详情将从 WooCommerce 拉取并经 `models/product.js` 映射后展示。
