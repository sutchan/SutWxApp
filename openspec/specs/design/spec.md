...
#### 4.1.4 品牌衍生渐变 token（v3.1.1 新增）

占位图 / 横幅所用的品牌绿渐变统一归档为衍生 token，禁止在业务代码中散落硬编码渐变色值：

| Token | 色标对 | 用途 |
|-------|--------|------|
| `--grad-leaf-1` ~ `--grad-leaf-6` | 浅绿系两档色标（如 `#F1F8E9, #C8E6C9`、`#E8F5E9, #DCEDC8`、`#F1F8E9, #A5D6A7` 等） | 商品占位图 / SVG 叶形插画底渐变 |
| `--grad-banner-1` ~ `--grad-banner-3` | 取自 leaf 系列 | 首页 banner 氛围渐变（135deg） |
| `--grad-user-header` | `#2E7D32, #1B5E20` | "我的"页头部品牌渐变 |

用法：`linear-gradient(135deg, var(--grad-banner-1))`；token 值为逗号分隔的两档色标。新增任何强调渐变须先归档到本节，再引用。
...
| 版本 | 更新日期 | 更新内容 | 作者 |
|------|----------|----------|------|
| 3.1.1 | 2026-09-12 | 色彩系统新增"品牌衍生渐变 token"子小节（--grad-leaf-1~-6 / --grad-banner-1~-3 / --grad-user-header），归档占位图与横幅渐变，与高保真原型同步 | SutWxApp Design（彩格调） |
